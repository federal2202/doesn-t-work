...
class OneWin:
    ...
    async def main(self):
        try:
            ...
            if login == False:
                ...
                return 0
            logger.info(f"main | Thread {self.thread} | {self.account} | Start! | PROXY : {self.proxy}")
            while True:
                try:
                    ...
                    logger.error(f"main | Thread {self.thread} | {self.account} | {err}")
    ...
    async def upgrade(self, name):
        ...
        for num in range(10):
            tool = tool.replace(str(num),'')
        logger.success(f"upgrade | Thread {self.thread} | {self.account} | upgrade {tool} to {answer.get('NextLevel').get('level')-1} LEVEL")
    ...
    async def everydayreward(self):
        ...
        if is_collected == False:
            ...
            logger.success(f"everyday | Thread {self.thread} | {self.account} | claim {day} reward : {reward}")
    ...
    async def login(self):
        try:
            ...
            logger.error(f"login | Thread {self.thread} | {self.account} | {err}")
    ...
    async def get_tg_web_data(self):
        ...
        try:
            ...
            if 'USER_DEACTIVATED_BAN' in str(err):
                logger.error(f"login | Thread {self.thread} | {self.account} | USER BANNED")
